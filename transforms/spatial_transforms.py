...
class Compose(object):
    ...
    def randomize_parameters(self, c_size=0, index=0):
        for t in self.transforms:
...
class CornerCrop(object):
    ...
    def __init__(self, size, crop_position=None, interpolation=Image.BILINEAR):
        self.size = size
...
class MultiScaleRandomCropMultigrid(object):
    ...
    def randomize_parameters(self, c_size, index=0):
        self.size = c_size
        self.scale = self.scales[random.randint(0, len(self.scales) - 1)]
        self.tl_x = random.random()
        self.tl_y = random.random()
