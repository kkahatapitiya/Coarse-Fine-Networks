...
parser.add_argument('-root', default='/home/kkahatapitiy/data/Charades_v1_rgb', type=str)
...
# 0.005 with 8*4 for 25 epochs
BS = 6 #16 #9 #15 #10 # 16 #16
BS_UPSCALE = 1
INIT_LR = 0.01 #0.02 * BS_UPSCALE #* 0.01
X3D_VERSION = 'M'
...
def run(init_lr=INIT_LR, warmup_steps=0, max_epochs=200, mode='rgb', root='/home/kkahatapitiy/data/Charades_v1_rgb', #'/nfs/bigdisk/kumarak/datasets/charades/Charades_v1_rgb',
    train_split='data/charades.json', batch_size=BS*BS_UPSCALE, frames=80,
    x3d_out = ['','']
    ): #, save_model=''):
    # setup dataset
    ...
    iterations_per_epoch = 7900//batch_size # *num_steps_per_update
    val_batch_size = batch_size#//5*2 #5 #batch_size//3 #*2 #//2 # //8 for 10 crop with 4 gpus, //2 otherwise
    val_iterations_per_epoch = 1850//val_batch_size
    ...
    if mode == 'flow':
        x3d = resnet_x3d.generate_model(x3d_version=X3D_VERSION, n_classes=400, n_input_channels=2, base_bn_splits=1)
        #save_model = 'models/flow_temp_'
    else:
        AUX_LOSSES = None #'flow'
        ...
        '''load_ckpt = torch.load('models/x3d_coarse_fine_charades_loc_rgb_2stream_sgd_057000.pt') # 032000 mAP: 0.2166 --> mAP: 0.2188
    ...
    if steps>0:
        ...
        x3d.load_state_dict(load_ckpt['model_l_state_dict'])
        x3d_g.load_state_dict(load_ckpt['model_g_state_dict'])
        #state = x3d.state_dict()
        #state.update(load_ckpt['model_state_dict'])
        #x3d.load_state_dict(state)
    ...
    optimizer = optim.Adam([{'params': base_params}], lr=lr, weight_decay=1e-5)
    ...
    while epochs < max_epochs:#for epoch in range(num_epochs):
        ...
        for phase in 2*['train']+['val']: #*5+['val']:
            ...
            for i,data in enumerate(dataloaders[phase]):
                ...
                masks_g = masks_gl[:,::gamma_tau*2] # g clip mask
                ...
                feat_g, _ = x3d_g([inputs_g, masks_g]) # B C T
                ...
                #loss = 1 * (cls_loss + loc_loss + cls_loss_g + loc_loss_g)/(4 * num_steps_per_update)
